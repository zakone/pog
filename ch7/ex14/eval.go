...
func (o operand) Eval(env Env) float64 {
    switch o.fn {
    case "max":
        return math.Max(o.args[0].Eval(env), o.args[1].Eval(env))
    case "min":
        return math.Min(o.args[0].Eval(env), o.args[1].Eval(env))
    }
    panic(fmt.Sprintf("unsupported function call: %s", o.fn))
}
...
func (o operand) Check(vars map[Var]bool) error {
    arity, ok := operParams[o.fn]
    if !ok {
        return fmt.Errorf("unknown function %q", o.fn)
    }
    if len(o.args) != arity {
        return fmt.Errorf("call to %s has %d args, want %d",
            o.fn, len(o.args), arity)
    }
...
var operParams = map[string]int{"max": 2, "min": 2}
